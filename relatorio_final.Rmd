...
###################### explicar melhor árvore de decisão
...
A parametrização da Beta é $R \sim Beta(\alpha, \beta)$, em que os estimadores pelo método dos momentos são $\hat{\alpha} = \bar{R} \left(\frac{\bar{R}(1 - \bar{R})}{S_R^2} - 1\right)$ e $\hat{\beta} = (1-\bar{R}) \left(\frac{\bar{R}(1 - \bar{R})}{S_R^2} - 1\right)$ \cite{degroot2012probability}. Os parâmetros serão estimados através do método dos momentos com o uso de revisão (do rendimento) dos exames na literatura, quando disponível, e do uso de diversos artigos que determinem o rendimento do exame.
...
É possível perceber através da Figura \ref{fig:RCEI} que o uso do microarray e exoma como exames pilotos tem potencial de alta redução dos custos (entre R\$1.000 e R\$2.000), com uma pequena redução da efetividade (entre 1,5\% e 2\%), visto que para essas duas estratégias a análise de cariótipo não entra no rol de exames. Além disso, o exoma se mostra como o teste piloto mais vantajoso, pois, obteve em 74\% da simulação uma redução de custo maior ou igual que o microarray como teste piloto, e em 83\% um decrésimo na efetividade menor ou igual ao do microarray.

Outra observação que se pode fazer através da Figura \ref{fig:RCEI} é que para a análise de microarray o RCEI é muito mais sensível ao seu rendimento que ao seu custo, o que é indicado pelo formato "achatado" de sua elipse de confiança. Já paro o sequenciamento do exoma, o RCEI é sensível a ambos de maneira similar.


# Discussão

A árvore de decisão construída leva a acreditar que a aplicação do sequenciamento do exoma seguida de uma análise de microarray em caso de falha de diagnóstico é mais custo-efetiva que o contrário (análise de microarray antes do sequenciamento do exoma) ou a realização da análise de cariótipo seguida da análise de microarray e sequenciamento do exoma. A Análise de sensitividade reforçou essa conclusão, pois o exoma como exame piloto teve uma redução de custo maior ou igual que o microarray como teste piloto em 74\% da simulação e um decrésimo na efetividade menor ou igual ao do microarray em 83\%. Ela também indica que o custo-efetividade incremental (em relação ao uso de análise de cariótipo) é sensível a apenas o rendimento para o microarray e a ambos o custo e rendimento do exoma. Como é esperado que os custos para o sequenciamento do exoma diminuam (*c.f.* Figura \ref{fig:custo}), é esperado, portanto, que o custo-efetividade incremental seja cada vez menor, sendo o sequenciamento do exoma cada vez mais vantajoso em comparação com o uso da análise de cariótipo como exame piloto.
